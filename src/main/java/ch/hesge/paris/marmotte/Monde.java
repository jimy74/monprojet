...
public class Monde extends FramePerso {
    ...
    /**
     * Doit impérativement trouver une case aléatoirement si toutesCasesPleines()
     *
     * @param alea
     * @return
     */
    public Case getCaseVideAliatoire(Random alea) {
        ...
        while (true) {
            for (Case uneCase : getCases()) {
                //Si la case est bien vide et à la position trouvée aléatoirement
                if (uneCase.isVide() && (alea.nextInt(getTailleX()) + 1 == uneCase.getPositionX() && (alea.nextInt(getTailleY()) + 1 == uneCase.getPositionY()))) {
